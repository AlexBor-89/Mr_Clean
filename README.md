...
**Mr. Clean** — это автоматизированная программа для очистки файлов и каталогов на компьютере.
...
### Описание
Программа предназначена для автоматической очистки файлов и каталогов по заданным правилам.  
...
Файл `values.ini` содержит правила очистки для разных каталогов. Каждое правило находится в своей секции.

#### Структура секции:
```
[Name_1]
Path = "Путь к очищаемому каталогу"
...
где:
- **Name** - это произвольное имя секции, которое идентифицирует очищаемый каталог.
- **Path** - это путь к очищаемому каталогу, который может содержать переменные среды, например **%%TEMP%%**, **%%UserProfile%%** и т. д.
  - Путь должен быть заключён в кавычки, а переменные среды в двойные символы процента.
- **Method** - это метод удаления, который определяет, какие каталоги и файлы будут удалены. Возможные значения:
...
- **Days** - это количество дней, за которые нужно оставить каталоги и файлы и не удалять их. Должно быть положительным целым числом.
...
Допустим, вы хотите очистить каталог загрузок пользователя от `.exe`, `.txt` и `.csv` файлов, старше 7 дней.  
...
Программа просканирует каталог Загрузки и удалит все каталоги и файлы, дата последнего доступа к которым была более 7 дней назад.
...
- Добавлена кнопка "Завершить работу" в графическом интерфейсе для принудительного завершения очистки
- Исправлена проблема с путями к конфигурационным файлам при запуске программы из терминала
- Исправлена проблема игнорирования ошибок доступа и остановки при использовании метода 0
- Улучшена обработка событий закрытия программы, включая проверку видимости иконки в трее
- Улучшена обработка исключений при выполнении методов
- Улучшена логика очистки обработчиков логов
...
- Добавлен новый метод очистки (4 - удаляет все файлы в каталоге и в подкаталогах, с сохранением структуры каталогов)
